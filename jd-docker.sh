...
if [ "$NETWORK" = "bridge" ]; then
    echo -n -e "\e[36m输入自定义的映射端口[默认为：5700]->\e[0m"
    read jd_port
    JD_PORT=$jd_port
    if [ -z "$jd_port" ]; then
        JD_PORT=5700
...
log "5.安装已经完成。\n现在你可以访问设备的 ip:5700 用户名：admin  密码：adminadmin  来添加cookie，和其他操作。感谢使用！"
